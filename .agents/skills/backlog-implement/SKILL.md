...
description: Implement an Axym backlog plan end-to-end (or a selected epic) from product/PLAN_NEXT.md (or a specified plan), with strict branch bootstrap, per-story test wiring, CodeQL validation, and scoped final revalidation. No issue/PR automation.
disable-model-invocation: true
---

# Plan Implementation (Axym)
...
- In the allowlist case, require current branch is already `main`, run step 1, skip steps 2-3, and create the branch from fetched `origin/main` with `git checkout -b codex/<plan-scope> origin/main` to preserve plan edits on an up-to-date base.
...
- `axym regress run --baseline <baseline-path> --frameworks eu-ai-act,soc2 --json` for policy-story contract checks.
- `axym verify --chain --json` for artifact-story integrity checks.
