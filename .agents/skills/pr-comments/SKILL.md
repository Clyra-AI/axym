...
# PR Comments Implement + Ship (Axym)
...
- `axym verify --chain --json` (when chain/integrity paths are touched)
- `axym regress run --baseline <baseline-path> --frameworks eu-ai-act,soc2 --json` (when policy/regress paths are implicated)

## Axym Priorities for Fixes
