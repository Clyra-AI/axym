---
name: cut-release
description: Cut a new Axym release tag directly from main, monitor release/post-release validation, and run up to 2 hotfix PR loops when failures are actionable.
...
- `gh auth status`
- `gh workflow view release --repo Clyra-AI/axym`
- `gh secret list --repo Clyra-AI/axym | awk '{print $1}' | rg '^HOMEBREW_TAP_GITHUB_TOKEN$'`
...
- confirm run URL and terminal status, for example:
  - `gh run list --repo Clyra-AI/axym --workflow release --limit 5`
  - `gh run watch --repo Clyra-AI/axym <run-id>`
...
- `AXYM_UAT_RELEASE_VERSION=<version> bash scripts/test_uat_local.sh`
...
- use `gh run list --repo Clyra-AI/axym --branch main --limit 10`
...
- `AXYM_UAT_RELEASE_VERSION=<new-version> bash scripts/test_uat_local.sh`
...
Capture release diagnostics using `axym` commands with `--json`, for example:
