...
- `axym collect --dry-run --json` before ship to capture machine-readable local readiness evidence.
- `axym verify --bundle <artifact-path> --json` when validating artifact integrity in a failing CI path.
- `axym regress run --baseline <baseline-path> --frameworks eu-ai-act,soc2 --json` when policy-path checks are implicated.
