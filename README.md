...
- Sibling ingest: `./axym ingest --source wrkr --json --input ./wrkr-records.jsonl` and `./axym ingest --gait-pack ./gait-pack --json`. For Gait v1.5 lifecycle evidence, add `--source gait --gait-lifecycle-verification ./verification.json`; the caller-owned config supplies the trusted key, exact lineage digests, validity window, and fixture policy. The public config contract is [lifecycle-verification-config-v1.schema.json](schemas/v1/gait/lifecycle-verification-config-v1.schema.json), with a safe illustrative [example](schemas/v1/gait/lifecycle-verification-config.example.json). Fixture-signed packs remain non-authoritative and are never appended as ordinary Proof records.
