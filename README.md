...
- Sibling ingest: `./axym ingest --source wrkr --json --input ./wrkr-records.jsonl` and `./axym ingest --gait-pack ./gait-pack --json`. For Gait v1.5 lifecycle evidence, add `--source gait --gait-lifecycle-verification ./verification.json`; the caller-owned config supplies the trusted key, exact lineage digests, validity window, and fixture policy. Fixture-signed packs remain non-authoritative and are never appended as ordinary Proof records.
