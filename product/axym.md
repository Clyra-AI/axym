...
- **Evidence-set mapping contract.** Proof `v0.7.0` may define alternative `evidence_sets` for a control. Axym evaluates the sets as OR alternatives while requiring every `required_record_type` within one set and applying its `source_products`, `required_fields`, and `minimum_frequency` together. If no set is complete, Axym selects the closest partial or gap deterministically by matched-type completeness and missing requirements. Machine output includes `REQUIRED_RECORD_TYPES_NOT_MET` for incomplete multi-type sets and prefixes the selected-set rationale with `evidence_set=<id>`. Bundle generation and verification consume that same selected requirement set rather than reinterpreting or flattening Proof's alternatives.
