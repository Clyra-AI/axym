...
- Action Contract compatibility: `axym action-contract consume <proposed_action_contract.json> --json` validates exactly one Wrkr v3 proposal, preserves exact producer-native bytes/refs in `--store-dir`, and reports a non-authoritative receipt conforming to Axym's versioned consumer-receipt schema. Optional exact Gait activation verification uses `--activation`, `--selection`, `--trusted-key`, and `--verify-at`. The standalone conformance entrypoint is `axym-action-contract-consumer <proposed_action_contract.json>`; see [the versioned schema contract](../../schemas/v1/action_contract/README.md).
...
- `axym record add --input <record.json> --json`: validates the public manual-input contract, normalizes compatibility-only `record_version: "1.0"` payloads to `v1`, then signs and appends the record to the local chain. Caller-authored `gait_lifecycle` aggregates are rejected; authoritative lifecycle evidence must use the verified Gait ingest path, which attaches a signed verification receipt bound to the exact aggregate before it enters the chain. Shared proof-record semantics remain owned by `Clyra-AI/proof`; see [../../schemas/v1/record/README.md](../../schemas/v1/record/README.md).
...
- Verified lifecycle bundles carry `gait-verified-lifecycle.json` entries containing the signed receipt, persisted record hash, source artifact digest, and evidence-set identity. Bundle build and verify, plus `governance timeline|graph|replay`, reverify this receipt before promoting lifecycle axes; missing or changed receipts remain gaps.
- `axym action-contract consume <path> --json`: consumes one report-only Wrkr proposal, checking optional expiry against current UTC time (or `--verify-at`); it never treats the proposal as execution authority and does not claim execution/effect/containment evidence.
- `axym ingest --source action-contract --input <path> --store-dir <store> --json`: retains exact producer-native proposal bytes and a digest-bound reference envelope without adding execution authority.
- `axym ingest --source action-contract --input <path> --verify-at <RFC3339> --json`: validates optional proposal expiry against the explicit evaluation time; the default is the current UTC time.
- `axym governance timeline|graph|replay --store-dir <store> --contract-id <id> [--format json|markdown]`: projects deterministic lifecycle views through the governance reducer and preserves explicit gap states. Source execution transitions (including blocked runs), rejected or unresolved effects, requested/partial/unresolved containment, and compensation required/not-required/started/completed/failed states are mapped consistently in timeline and graph output; required, blocked, partial, or unresolved outcomes never claim completion.
- The quarantined producer fixture at `testdata/governance/v1/producer-fixture` is regenerated with `go run ./scripts/generate_governance_fixture.go --update`; `--check` performs an offline exact-byte/source/orphan check, and `go test ./core/governance -run CheckedInProducerFixturePack` verifies the pinned cross-product joins.
